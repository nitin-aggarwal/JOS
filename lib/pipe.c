...
pipe(int pfd[2])
{
	...
	if (debug)
		cprintf("[%08x] pipecreate va %08x pa %08x\n", env->env_id, va, vpt[VPN(va)]);
		
	//cprintf("fd0 va: %08x pa: %08x fd1 va: %08x pa:%08x ***************\n",fd2data(fd0),vpt[VPN(fd2data(fd0))],fd2data(fd1),vpt[VPN(fd2data(fd1))]);
	pfd[0] = fd2num(fd0);
	pfd[1] = fd2num(fd1);
	return 0;
...
devpipe_read(struct Fd *fd, void *vbuf, size_t n)
{
	...
	//cprintf("before in pipe.c read data fd address:\n");

	p = (struct Pipe*)fd2data(fd);
	//cprintf("after in pipe.c read data fd address: %x08x\n",p);
	if (debug)
