...
spawn(const char *prog, const char **argv)
{
	...
	if(debug)
		cprintf("End of spawn returns [%08x]\n", child);
...
copy_shared_pages(envid_t child)
{
	...
	if(debug)
		cprintf("In copy shared in spawn******************************\n");
                ...
                for(j=0;j<NPTENTRIES;j++)
		{
                        ...
                        {
				addr = (void *) (pagenum * PGSIZE);
				if(PTE_SHARE & vpt[pagenum])
					r = sys_page_map(0, addr, child, addr,  (vpt[pagenum] & PTE_USER)); // used PTE_USER & vpt[pagenum]   
                                if(r < 0)
