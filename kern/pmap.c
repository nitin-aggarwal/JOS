...
boot_alloc(uint32_t n, uint32_t align)
{
	...
	//rahul	
	v = ROUNDUP(boot_freemem, align);		//using roundup mentioned in types.h to obtain allocated chunk
	boot_freemem = (char *) ((uint32_t)v + n);	//increaing boot_freemem to record allocation
	
	//cprintf("Allocated 0x%08x , Current bootfreemem 0x%08x , END 0x%08x \n", v, boot_freemem, end);
	if (PADDR(boot_freemem) > maxpa)	//CHECK IF MAX ADDRESS ASSUMED IS CORRECT or MAXPA shd be used
	{
		panic("OUT OF MEMORY - current 0x%08x , max 0x%08x", boot_freemem, maxpa);
		return NULL;
	}
	return v;					//returning allocated chunk
...
i386_vm_init(void)
{
	...
	//cprintf("1111 0x%08x 0x%08x",boot_pgdir[0],PTE_U);
	//////////////////////////////////////////////////////////////////////
	...
	//rahul
	size_t npage_size;
	npage_size = npage * sizeof(struct Page);
	pages = (struct Page *) boot_alloc(npage_size, PGSIZE);
	memset(pages, 0, npage_size);
	
	
	//cprintf("2222 0x%08x 0x%08x",boot_pgdir[0],PTE_U);
	//////////////////////////////////////////////////////////////////////
	...
	envs = (struct Env *) boot_alloc(envs_size, PGSIZE);
	...
	//cprintf("3333 0x%08x 0x%08x",boot_pgdir[0],PTE_U);
	check_page_alloc();
	
	//cprintf("4444 0x%08x 0x%08x",boot_pgdir[0],PTE_U);
	page_check();
	...
	//rahul
	boot_map_segment(pgdir,UPAGES,npage_size,PADDR(pages), PTE_U);
	...
	//lab3
	boot_map_segment(pgdir, UENVS, envs_size, PADDR(envs), PTE_U|PTE_P);	
...
page_init(void)
{
	...
	int i;
	//rahul
	//CHECK IF KERBASE NEEDS TO BE ADDED AND CHECK IF MARKING 0 IS CORRECT FOR IN USE  //have to mark 1 for in use
	//rahul set everything to 1	

	//cprintf("test1\n");
	LIST_INIT(&page_free_list);
	pages[0].pp_ref = 0;
	for (i = 1; i < npage; i++) {
		//cprintf("0x%08x , 0x%08x , 0x%08x , 0x%08x \n", page2pa(&pages[i]), EXTPHYSMEM, PADDR(boot_freemem));
		if(page2pa(&pages[i]) >= (IOPHYSMEM) && page2pa(&pages[i]) <= (EXTPHYSMEM))
		{
			pages[i].pp_ref = 0;
		}
		else if(page2pa(&pages[i]) > (EXTPHYSMEM) && page2pa(&pages[i]) < PADDR(boot_freemem))
		{
			pages[i].pp_ref = 0;
		}
		else
...
page_alloc(struct Page **pp_store)
{
	...
	//cprintf("inside page_alloc\n");
	if(LIST_EMPTY(&page_free_list))
	{
		//cprintf("No free pages available, LIST_EMPTY returns %d\n", LIST_EMPTY(&page_free_list));
		return -E_NO_MEM;
	}
	else
	{
		allocated_page = LIST_FIRST(&page_free_list);
		LIST_REMOVE(allocated_page,pp_link);	
		page_initpp(allocated_page);
	
		*pp_store = allocated_page;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	temp_pde = &pgdir[PDX(va)];
	if(*temp_pde & PTE_P)
	{
		temp_pte = (pte_t *)(KADDR(PTE_ADDR(*temp_pde)));
		return ((pte_t *) temp_pte + PTX(va));
	}
	else
	{
		if(create == 0)
		...
		{
			//Allocate a page
			if(page_alloc(&new_page))
			{return NULL;}
			else
			{
				new_page->pp_ref = 1 ;	//rahul changed += 1 to = 1
				temp_pte = page2kva(new_page);
				memset(temp_pte, 0, PGSIZE);
				
				*temp_pde = PTE_ADDR(PADDR(temp_pte))|create|PTE_W|PTE_P|PTE_U;
				return ((pte_t *) temp_pte + PTX(va)); 	
			}
		}
	}
	//cprintf("a = 0x%08x b = 0x%08x c = 0x%08x", &pgdir, &temp_pde, *temp_pde);
	//temp_pte = (pte_t *) (*temp_pde);
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm) 
{
	...
	temp_pte = pgdir_walk(pgdir,va,0);

	if(temp_pte)
	{
		//Page entry exists and page is re-inserted at same va
		if((*temp_pte & PTE_P) && pa2page(PTE_ADDR(*temp_pte)) == pp)
		{
			pp->pp_ref -= 1;
			perm = perm & 0xfff;
			*temp_pte &= 0xfffff000;
		...
		if(*temp_pte & PTE_P)
		{
			...
			*temp_pte = PTE_ADDR(page2pa(pp))|perm;
			tlb_invalidate(pgdir,va);
			return 0;
		}
	}
	...
	{
		temp_pte = pgdir_walk(pgdir, va, perm);
		if(!temp_pte)
		{
			pp->pp_ref -= 1;
			return -E_NO_MEM;
...
boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	{
		pte_t *temp_pte = pgdir_walk(pgdir, (void *)(la + n), perm);
		*temp_pte = PTE_ADDR(pa + n)|perm;	
	}
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	temp_page = page_lookup(pgdir,va, &temp_pte);
	if(!temp_page)
	{
		return;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	perm = perm |PTE_P;
	j = (uint32_t)va;
	for(i=j;i < j+len;i=i+1)
	{
		user_mem_check_addr = (uintptr_t) i;
		if(i >= ULIM)
			return -E_FAULT;
		pde = (pde_t *)env->env_pgdir + PDX(i);
		if(pde)
		{
			if(perm & PTE_U)
			{
...
page_check(void)
{
	...
	//cprintf("0x%08x 0x%08x",boot_pgdir[0],PTE_U);
